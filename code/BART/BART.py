...
def main():
  ...
  if runMCMC < 1:  # Pressure file
    mp.makeP(n_layers, p_top, p_bottom, date_dir+press_file, log)
  ...
  if uniform is not None and runMCMC < 8:
    ...
    temp = ipt.initialPT2(date_dir, PTinit,         date_dir+press_file, 
                          PTtype,   PTfunc[PTtype], tep_name)
    # Generate the uniform-abundance profiles file:
    mat.uniform(date_dir+atmfile, date_dir+press_file, abun_basic, tep_name,
  ...
  if runMCMC < 4:  # Pre-atmospheric file
    ...
    raw_input("  open Initial PT profile figure and\n" 
              ...
              "PT parameters.")
    mat.make_preatm(tep_name, date_dir+press_file, date_dir+abun_file, 
                    in_elem, out_spec, date_dir+preatm_file, temp)
  ...
  if runMCMC < 8:  # Atmospheric file
    ...
    # Execute TEA:
    mu.msg(1, "\nExecute TEA:")
    proc = subprocess.Popen([TEAcall, date_dir+preatm_file, 'TEA'])
    proc.communicate()

    shutil.copy2(date_dir+"TEA/results/TEA.tea", date_dir+atmfile) 
    # Add radius array:
    mat.makeRadius(out_spec, date_dir+atmfile, date_dir+abun_file, 
                   tep_name, refpress)
